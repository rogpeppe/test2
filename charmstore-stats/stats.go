...
func (s *Store) ArchiveDownloadCounts(id *charm.URL) (thisRevision, allRevisions AggregatedCounts, err error) {
	...
	thisRevision, err = s.statsCacheFetch(&fetchId)
	if err != nil {
		return AggregatedCounts{}, AggregatedCounts{}, errgo.Mask(err)
	}

	fetchId.Revision = -1
	allRevisions, err = s.statsCacheFetch(&fetchId)
	if err != nil {
		return AggregatedCounts{}, AggregatedCounts{}, errgo.Mask(err)
	}
	return
...
func (s *Store) statsCacheFetch(id *charm.URL) (AggregatedCounts, error) {
	prefix := id.Revision == -1
	kind := params.StatsArchiveDownload
	if id.User == "" {
		kind = params.StatsArchiveDownloadPromulgated
	}
	return s.aggregateStats(EntityStatsKey(id, kind), prefix)
}
